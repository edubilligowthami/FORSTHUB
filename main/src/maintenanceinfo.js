...
function Maintenanceinfo(){
    ...
    const handlesubmit=(e)=>{
        e.preventDefault();
        console.log(request)
        axios.post('http://localhost:5000/addmaintenancedata',{request}).then((result)=>{console.log(result.body)}, alert('data added to inventory successfully'), window.location.href='/adminhome')
    }
    ...
    return(
        <>
            ...
            <Container>
                ...
                <FormGroup>
                    <tr>
                        <td><Label style={{align:'left', fontWeight:'bold'}}>Remarks:</Label></td>
                        
                    </tr>
                    <Input name="remarks"  type="text"  onChange={(e)=>setRequest({...request,remarks:e.target.value})}/>  
                </FormGroup>
                </Col>
                </Row>
                <Row>
